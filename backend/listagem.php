<?php 
  ...
  if(count($_GET)>0) {
    ...
    if($acao == "listaAutor" || $acao == 'cadastroLivro') {
    ...
    if($acao == "listaEditora" || $acao == 'cadastroLivro') {
    ...
    if($tabela == "Livro") {
      ...
      if($acao == "listaLivro") {
        ...
        while($row = $results->fetch_assoc()) {
          ...
          $sql = "SELECT nome, Autor.id_autor as 'id_autor' FROM Autor INNER JOIN Autor_Livro ON Autor.id_autor = Autor_Livro.id_autor WHERE id_livro = " . $row["id_livro"];
